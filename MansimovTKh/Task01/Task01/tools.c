...
void read_library(const char* filename, book_info** books, int* lines) {
	...
	*books = (book_info*)malloc(*lines * sizeof(book_info));   //выделение памяти для lines строк
	...
	for (i = 0; i < (*lines); i++) {
		...
		char* a = strtok(buffer, ";"); //возвращает указатель на имя автора
		char* t = strtok(NULL, ";");   //возвращает указатель на имя книги
		char* p = strtok(NULL, ";");   //возвращает указатель на имя издательства
		char* year_str = strtok(NULL, ";");  //возвращает указатель на год
		int y;

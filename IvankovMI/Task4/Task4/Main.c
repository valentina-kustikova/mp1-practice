...
int main() {
	int n, sl[5] = {-1}, mysl[5] = {-1}, i, j, cows = 0, bulls = 0, ok = 0, try = 1;  //на всякий случай инициализирую массивы, пока не знаю, зачем
	...
	while (!ok) {
		...
		if (bulls == n)
			...
			printf("??????•????????????¶§?????????? !\"#$%&'()*+,-./01234569:;<=>?@AB..YZ[\\]^_`ab..yz{|}~?АБВ..ЭЮЯабв..клмноп????????????????????????????????????????????????рстуфхцчшщъыьэюяЁёЄєЇїЎў°?·?№¤? ??????•????????");
			//repl = (try < 2) ? "Вы не отгадали слово, но в нем случайно затесались " : "В вашем слове";
